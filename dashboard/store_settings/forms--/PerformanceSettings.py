...
class PerformanceSettingsForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            # === CACHING ===
            'enable_page_cache': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'cache_duration_minutes': forms.NumberInput(attrs={
                'class': 'form-control',
                'min': 5,
                'max': 1440
            }),
            'enable_browser_cache': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'browser_cache_duration_days': forms.NumberInput(attrs={
                'class': 'form-control',
                'min': 1,
                'max': 365
            }),
            'enable_database_query_cache': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'cache_products': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'cache_categories': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'cache_settings': forms.CheckboxInput(attrs={'class': 'form-check-input'}),

            # === IMAGE OPTIMIZATION ===
            'enable_lazy_loading': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'enable_webp': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'webp_quality': forms.NumberInput(attrs={
                'class': 'form-control',
                'min': 1,
                'max': 100
            }),
            'enable_image_compression': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'image_quality': forms.NumberInput(attrs={
                'class': 'form-control',
                'min': 1,
                'max': 100
            }),
            'enable_responsive_images': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            ...
            # === MINIFICATION ===
            'minify_html': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'minify_css': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'minify_js': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'combine_css': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'combine_js': forms.CheckboxInput(attrs={'class': 'form-check-input'}),

            # === CDN ===
            'enable_cdn': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'cdn_provider': forms.Select(attrs={'class': 'form-select'}),
            'cdn_url': forms.URLInput(attrs={
                'class': 'form-control',
                'placeholder': 'https://cdn.example.com'
            }),
            'cdn_for_images': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'cdn_for_css': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'cdn_for_js': forms.CheckboxInput(attrs={'class': 'form-check-input'}),

            # === PRELOADING ===
            'enable_dns_prefetch': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'enable_preconnect': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'enable_prefetch': forms.CheckboxInput(attrs={'class': 'form-check-input'}),

            # === COMPRESSION ===
            'enable_gzip': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'enable_brotli': forms.CheckboxInput(attrs={'class': 'form-check-input'}),

            # === DATABASE ===
            'enable_database_connection_pooling': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            ...
            # === MONITORING ===
            'enable_performance_monitoring': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'log_slow_queries': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
