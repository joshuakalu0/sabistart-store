...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        checks.append(("platform_cname", settings.PLATFORM_CNAME not in {"", "localhost", "127.0.0.1"}, "Set PLATFORM_CNAME to the platform domain, for example sabistart.store."))
        checks.append(("subdomain_suffix", bool(settings.SUBDOMAIN_SUFFIX), "Set SUBDOMAIN_SUFFIX so tenant subdomains resolve correctly, for example .sabistart.store."))
